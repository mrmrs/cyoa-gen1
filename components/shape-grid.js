...
const ShapeGrid = ({ palette, gap = 16, width = 64, height = 64, lines = 16, xOffset = 0, yOffset = 0, colors, bgColor, color, strokeWidth, cols, rows,}) => {
      ...
      <>
        {[...Array(cols)].map((y,j) =>
          <>
            ...
            <g fill='transparent' stroke={palette[randomInt(0,palette.length-1)]} strokeWidth={strokeWidth}>
              <circle cx={j * (width / cols) + (width /cols/2) } 
              cy={i * width/cols} 
              r={randomInt(0,parseInt(width/cols/2 - 1))} fill={'url(#Gradient'+randomInt(0,15)+')'} strokeWidth={0} />
