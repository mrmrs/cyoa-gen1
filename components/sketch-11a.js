...
const Sketch9 = ({ colors, bgColor, color = 'red', maxLimit = randomInt(50,150), strokeWidth = randomInt(4,24), cols, rows, symmetrical, cellWidth, cellHeight}) => {
  ...
  const gridColor = colors[randomInt(0,colors.length-1)]
    ...
    return (
      ...
      <svg viewBox={'0 0 '+width+' '+height} stroke='white' width='1000' height='1410' className='transitions' style={{ padding: '10%', backgroundColor: 'rgba(250,250,24,0)',backgroundBlendMode: 'none',  mixBlendMode: 'darken', overflow: 'hidden', display: 'block', width: '100%', height: 'auto', }}>
        <PolarGraph radialLines={cols} circles={rows} colors={colors} bgColor={gridColor} />
        <PolarGraph radialLines={cols} circles={rows} colors={colors} bgColor={gridColor} />
      </svg>
      </div>
  );
